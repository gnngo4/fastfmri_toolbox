...
REGRESSOR_COMBINATIONS = {
    ...
    "min+motion6+wmcsf_compcor+scrub": [
        FrequencyRegressors,
        DriftRegressors,
        MotionParameters,
        CompCorRegressors,
...
def run_glm(
    ...
    fla = FirstLevelAnalysis(
        derivatives_dir=f"{out_dir}/{image_type}",
    ...
    fla.run_frequency_glm(
        save_windowed_bold=True, save_predicted=True, save_residual=False
    )

    if image_type == "CIFTI":
        convert_niftis_to_ciftis(
            f"{out_dir}/{image_type}", path_loader.bold_dtseries, TR
        )
        for f in temp_files:
            os.remove(f)

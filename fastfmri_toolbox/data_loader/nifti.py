...
class NiftiHandler:
    ...
    def to_array(
        ...
        ), f"{self.mask_path} is greater than 3-dimensions."
        ...
        if verbose:
            print(
                f"""Truncate: [{idx_start},{idx_end}]
Normalization: {normalization}"""
            )

        self.coords = np.where(
            (mask_data > 0) & (data[:, :, :, idx_start:idx_end].mean(axis=3) > 0)
        )
    ...
    def get_array_idx(self, x, y, z):
        if self.coords is None:
            assert (
                False
            ), "Class attribute [coords] is not initialized.\nMethod [to_array()] must be ran."
    ...
    def to_nifti(self, data, outfile):
        assert (self.coords[0].shape[0] == data.shape[0]) and (
            len(data.shape) in [1, 2]
        ), "Dimensions of `data` must be 1- or 2-D."
    ...
    def _save_nifti(self, data, outfile):
        img = nib.Nifti1Image(data, self.nii_img.affine, self.nii_img.header)
        nib.save(img, outfile)

    def _percent_signal_change(self, X):
        assert len(X.shape) == 2, "Input array must be 2-dimensions."

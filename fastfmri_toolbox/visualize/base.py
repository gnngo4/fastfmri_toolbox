...
class PlotSlices:
    ...
    def plot_base(self, cmap: str = "gray", base_vmax: Union[float, None] = None):
        ...
        for i, z_slice in enumerate(self.base_z_indices):
            ...
            if self.num_rows == 1:
                ...
                self.axes[col].text(
                    int(0.02 * x_max), int(0.1 * y_max), f"z={z_slice}", c="white"
                )
            else:
                ...
                self.axes[row, col].text(
                    int(0.02 * x_max), int(0.15 * y_max), f"z={z_slice}", c="white"
                )
    ...
    def _load_base_nifti(self, nifti: Union[str, Path, nib.Nifti1Image]) -> np.ndarray:
        ...
        if isinstance(nifti, nib.Nifti1Image):
            ...
            if not nifti.exists():
                raise FileNotFoundError(f"Error: file path does not exist [{nifti}]")
    ...
    def _validate_nifti(self, nifti: Union[str, Path, nib.Nifti1Image]) -> np.ndarray:
        ...
        assert (
            data.shape == self.base_data.shape
        ), f"Dimensions does not match with base image [{self.base_data.shape}]"

        return data

    def _get_z_indices(
        self, nifti_data: np.ndarray, z_slice_range: Union[None, Tuple[int, int]]
    ) -> List[int]:
        """
        ...
        z_slices = [
            i for i in range(num_slices) if not np.all(np.equal(nifti_data[:, :, i], 0))
        ]

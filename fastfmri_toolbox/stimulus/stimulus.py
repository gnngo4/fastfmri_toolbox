...
class Stimulus:
    ...
    def generate(
        ...
        assert delay > 0, f"Delay (seconds) between acquired fMRI volume and stimulus start time must be positive."
        ...
        # Stimulus OFF (@ end)
        stim_off_end = np.zeros((self.n_timepoints-stim_off_start.shape[0]-stim_on.shape[0],))
    ...
    def convolve(self, hrf: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        updated_stim_time = np.round(np.arange(0,self.TR*(self.n_timepoints+N_diff),self.TR),self._round_TR(self.TR))
        
        return (updated_stim_time, updated_stim, convolved_signal)
    
    def _read_stimulus_frequencies(
        self, 
        stim_frequencies: Union[float,List[float]]
    ) -> List[float]:
